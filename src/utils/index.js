...
export const responsive = (WrappedComponent) =>
    withTheme((props) => {
        const {theme} = props
        const Res = withSizes(sizeToProps({breakPoints: theme.brkpnts}))(WrappedComponent);
        return  <Res {...props}/>;
    })
...
export const validateEmail = (email) => email && email.trim() && EMAIL_REGEX.test(email.trim());